...
1.  You must create a module named `ShoppingCart`. The test imports it with
    ...
    ```
2.  The module must export a `class`, so we can run `const cart = new ShoppingCart()`
3.  Right after constructing a new object, calling `cart.getItems()` should return an empty array.
4.  We should be able to call `cart.addItem(itemName, quantity, price)`, which adds a new item to an internal array. Subsequent calls to `cart.getItems()` should return the added items. Items in the array should be in this format:
    ```js
    {
    ...
    ```
5.  Calling `cart.clear()` should remove all items from the items array.
...
2.  The exported `pathFind` should be a function. Declare this function with the `function` keyword (otherwise the tests cannot check if your function is recursive later)
3.  The function will be called with two parameters: `pathFind(path, object)`. The path is an array of strings and the object is an object. The array of strings refer to a sequence of properties on the object. Here are some examples of how it should work:
    ```js
    ...
    ```
4.  The function should be a pure function, neither of the inputs should be mutated.
5.  The function should be recursive. If you don't manage making it recursive, it's more important that it returns the expected outputs.
